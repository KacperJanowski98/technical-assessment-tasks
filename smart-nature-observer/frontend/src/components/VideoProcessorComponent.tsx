...
const VideoProcessorComponent: React.FC<VideoProcessorProps> = ({
  ...
  const currentFrameIndex = video.frames.findIndex(
    frame => frame.id === selectedFrameId
  );
  
  // Get the current frame or default to the first one
  const currentFrame = currentFrameIndex >= 0 
  ...
  const nextFrame = () => {
    if (currentFrameIndex < video.frames.length - 1) {
  ...
  const handleTimeSliderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Find the nearest frame to this timestamp
    ...
    <div className="video-processor">
      ...
      <div className="video-controls">
        <div className="transport-controls">
          <button onClick={prevFrame} disabled={currentFrameIndex <= 0}>
            <FaStepBackward />
          </button>
          ...
          <button onClick={nextFrame} disabled={currentFrameIndex >= video.frames.length - 1}>
            <FaStepForward />
          </button>
